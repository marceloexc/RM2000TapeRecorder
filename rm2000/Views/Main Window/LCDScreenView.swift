  ...
  var body: some View {
    HStack(alignment: .center) {  // entire view
      VStack(alignment: .leading) {  // left side of the screen
        HStack {  // top glpyhs)
          VStack(alignment: .leading, spacing: 4) {
            LCDTextCaptionWithGradient("STEREO 48.0kHz")
        ...
        VStack(alignment: .leading) {
          LCDTextBig(
            recordingState.sampleRecordAudioFormat.asString.uppercased())
      ...
      VUMeter()
        ...
        .colorEffect(
          Shader(
            function: .init(library: .default, name: "dotMatrix"), arguments: []
          )
        )
